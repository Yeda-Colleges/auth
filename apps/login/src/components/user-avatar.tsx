...
export function UserAvatar({ loginName, displayName, showDropdown, searchParams }: Props) {
  ...
  return (
    <div className={`flex h-full flex-row items-center border p-[1px] dark:border-white/20 ${userAvatarRoundness}`}>
      ...
      <span className="text-14px ms-4 max-w-[250px] overflow-hidden pe-4 text-ellipsis">
        <bdi>{loginName}</bdi>
      </span>
      ...
      {showDropdown && (
        <Link
          ...
          className={`ms-4 me-1 flex items-center justify-center p-1 transition-all hover:bg-black/10 dark:hover:bg-white/10 ${userAvatarRoundness}`}
        >
